...
#Gunluk Aylik ve Yillik olarak secilen dbnin backupini alir.
#istenilirse ikinci bir alana yedegi kopyalar 
...
$SecondBackupDirectory = $OctopusParameters['SecondBackupDirectory']
...
function KlasorOlustur() {
    ...
    $Yil=get-date -format yyyy
	$Ay=get-date -format MM
	$Gun=get-date -format dd
	
        $Klasor=$directory+"\"+$SchedulerFolder+"\"+$Yil+"\"+$Ay+"\"+$Gun
    
    Write-Host "Yedek klasoru olusturuluyor $Klasor"
	
    if(!(Test-Path -Path $Klasor )){
    	$dosya=New-Item -ItemType directory -Path $Klasor
    	Write-Host "Yedek klasoru olusturuldu $Klasor" 
	}
		else
	{
  		Write-Host "Yedek klasoru mevcut $Klasor" 
	}
...
function CreateDevices {
    ...
    if ($devices -eq 1){
        $deviceName = $dbName + "_backup_" + $timestamp + "_octopus" + $extension
        $targetPath = CreatDevice $smoBackupRestore $directory $deviceName
        $targetPaths.Add($targetPath)
    } else {
        for ($i=1; $i -le $devices; $i++){
            $deviceName = $dbName + "_backup_" + $timestamp + "_" + $i + "_octopus" + $extension
            $targetPath = CreatDevice $smoBackupRestore $directory $deviceName
            $targetPaths.Add($targetPath)
        }
    }
...
function BackupDatabase {
    ...
    $YedekKlasoru = KlasorOlustur $BackupDirectory
    $targetPaths = CreateDevices $smoBackup $devices $YedekKlasoru $dbName $incremental
    ...
    #İkinci yedek kopyasi icin 
    
        #Write-Host "Second Backup Copy"
        #    $IkinciYedekKlasoru = KlasorOlustur $SecondBackupDirectory
        #    Copy-Item $targetPaths -Destination $IkinciYedekKlasoru

        #Write-Host "Second Backup Copy completed successfully"
...
$timestamp = if(-not [string]::IsNullOrEmpty($Stamp)) { $Stamp } else { Get-Date -format yyyy_MM_dd_HHmmss }
